...
# Signature of the LAPACKE_dsysv function
cdef extern from "mkl_lapacke.h" nogil:
    int LAPACKE_dsysv(int matrix_layout, char uplo, int64_t n, int64_t nrhs,
                      double* a, int64_t lda, int64_t* ipiv,
                      double* b, int64_t ldb)

# Function for solving symmetric system of equations
def mkl_solver_symm(double[:,::1] A, double[:,::1] B):
    """
    Function that uses MKL's LAPACK DSYV routine to solve a symmetric system
    of equations.

    The system solved has the form: AX = B

    INPUTS:
    - A: double array (n x n) with the coefficient matrix info (symmetric).
    - B: double array (n x nrhs) with the right hand sides of the system.
    ...
    cdef int64_t lda, ldb, n, nrhs, matrix_layout
    cdef int64_t[:] ipiv_memview, i
    cdef char uplo  # Character indicating upper ('U') or lower ('L') triangular part of A
    
    matrix_layout = 101  # Row major
    uplo = b'L'          # Assuming lower triangular part of A is provided
